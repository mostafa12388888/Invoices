...
@endif
					...
					<div class="col-xl-12">
						<div class="card">
							...
							@can('اضافة فاتورة')
								...
								@endcan
								@can('تصدير EXCEL')
								<a class="btn btn-primary" href="exportsinvoices"
	...
	<div class="dropdown-menu tx-13">
		...
		@endcan
		@can('ارشفة الفاتورة')
		<a class="dropdown-item Transfer_invoice_archive" href="#"  data-invoice_id="{{ $value_invoices->id }}"
                                                            data-toggle="modal" data-target="#Transfer_invoice"><i
                                                                class="text-warning fas fa-exchange-alt"></i>&nbsp;&nbsp;نقل الي
                                                            الارشيف</a>   
															@endcan   
															@can('طباعةالفاتورة')                
		<a class="dropdown-item Transfer_invoice_archive" href="print_invoices/{{ $value_invoices->id }}"
                                                            ><i
                                                                class="text-warning fas fa-print"></i>&nbsp;&nbsp;طباعه الفاتوره </a> @endcan                    
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <form action="intoarchive/destroy" method="post">
                        {{ method_field('delete') }}
                        {{ csrf_field() }}
                </div>
                <div class="modal-body">
                    هل انت متاكد من عملية الارشفة ؟
                    <input type="hidden" name="invoice_id" id="invoice_id_for_archive" value="">
...
<script>
	$('.Transfer_invoice_archive').on('click',function(){
		$d=document.getElementById('invoice_id_for_archive');
		$d.value=this.getAttribute('data-invoice_id');
